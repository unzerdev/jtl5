...
<div data-unzer-ui-component='{
    ...
    {if $hpPayment.allowSave && !empty($hpPayment.savedInfo)}
        <div class="mb-2">
            <div class="card-title h3">{lang key='s360_hp_savePaymentDataSepa' section="s360_unzer_shop5"}</div>
            {radiogroup stacked=true class="saved-payment-form-group"}
                ...
                {radio name="paymentData[resourceId]" value="new" id="saved-payment-new"}
                    <span class="content">
                        <strong>{lang key='s360_hp_newPaymentDataSepa' section="s360_unzer_shop5"}</strong>
                    </span>
                {/radio}
            {/radiogroup}
        </div>
    ...
    <unzer-payment publicKey="{$hpPayment.publicKey}" locale="{$hpPayment.locale}">
        <unzer-sepa-direct-debit credentialsOnFile="{if $hpPayment.allowSave}true{else}false{/if}" />
